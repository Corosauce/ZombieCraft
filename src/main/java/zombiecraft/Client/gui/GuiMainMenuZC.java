    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        for (int first = 0; first < options.keyBindings.length && !conflict; first++) {
	        for (int second = 0; second < options.keyBindings.length; second++) {
	            if (second != first && (options.keyBindings[second].getKeyDescription().contains("ZC_") || options.keyBindings[first].getKeyDescription().contains("ZC_")) && options.keyBindings[second].getKeyCode() == options.keyBindings[first].getKeyCode())
	            {
	                conflict = true;
	                break;
	            }
