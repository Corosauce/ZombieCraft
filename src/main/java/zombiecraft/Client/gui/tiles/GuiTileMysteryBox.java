...
public class GuiTileMysteryBox extends GuiTileBase {
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		this.drawString(FMLClientHandler.instance().getClient().fontRenderer, "Mystery Box" + (guiCur.equals("main") ? "" : " - SubGUI: " + guiCur), xStart+7, yStart-9, 16777215);
		
		if (guiCur.equals("main")) {
			this.drawStringRightAligned(FMLClientHandler.instance().getClient().fontRenderer, "Purchase time ticks", xStart+xSize/3, yStart2+yDiv, 16777215);
			this.drawStringRightAligned(FMLClientHandler.instance().getClient().fontRenderer, "Cycle time ticks", xStart+xSize/3, yStart2+yDiv*2, 16777215);
			
			this.drawString(FMLClientHandler.instance().getClient().fontRenderer, "Items to Randomize", xStart+7, yStart+8+60, 16777215);
			this.drawString(FMLClientHandler.instance().getClient().fontRenderer, "Inventory", xStart+7, yStart+8+148, 16777215);
		}
	}
    ...
    public void initGui()
    {
        ...
        if (guiCur.equals("main")) {
        	addTextBox(tEnt.nbtStrTimeoutPurchase, new GuiTextFieldZC(tEnt.nbtStrTimeoutPurchase, FMLClientHandler.instance().getClient().fontRenderer, xStartPadded + rightStart, yStartPadded, 80, 20));
        	addTextBox(tEnt.nbtStrTimeoutRandomize, new GuiTextFieldZC(tEnt.nbtStrTimeoutRandomize, FMLClientHandler.instance().getClient().fontRenderer, xStartPadded + rightStart, yStartPadded+btnHeightAndPadding*1, 80, 20));
        }
