...
public class RenderZCImp extends RenderBiped
{
    public RenderZCImp()
    {
    ...
    public void renderDebug(BaseEntAI par1Entity, double par2, double par4, double par6, float par8, float par9) {
	    ...
	    Entity sEnt = MinecraftServer.getServer().worldServerForDimension(ZCGame.ZCDimensionID).getEntityByID(par1Entity.getEntityId());
		
		if (sEnt != null && sEnt instanceof BaseEntAI) {
			BaseEntAI z = (BaseEntAI)sEnt;
			...
			if (pe != null/*par1Entity.currentAction != null*/) {
				//if () {
					this.func_147906_a(par1Entity, String.valueOf(pe.getCurrentPathIndex() + " - " + z.agent.jobMan.getPrimaryJob().state), par2, par4, par6, 999);
				//}
			} else {
