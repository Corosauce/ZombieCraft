...
public class TowerBase {
	...
	public void tickUpdate() {
		...
		if (targetAttack == null || tEnt.getWorldObj().getWorldTime() % 10 == 0) {
			boolean found = false;
			EntityLivingBase clEnt = null;
			double closest = 9999D;
	    	List list = tEnt.getWorldObj().getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(tEnt.xCoord, tEnt.yCoord, tEnt.zCoord, tEnt.xCoord+1, tEnt.yCoord+1, tEnt.zCoord+1).expand(range, range/2, range));
