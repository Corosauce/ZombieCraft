...
public class BaseEntAI extends EntityLiving implements ICoroAI, IAdvPF
{
    ...
    protected void updateArmSwingProgress() {
    	...
    	//ItemStack is = this.getCurrentEquippedItem();
    	if (this.getEquipmentInSlot(0) != null && this.getEquipmentInSlot(0).getItem() instanceof ItemGun/*agent.useInv && agent.entInv.getCurrentEquippedItem() != null && agent.entInv.getCurrentEquippedItem().getItem() instanceof ItemGun*/) {
    		if(agent.swingArm) {
    			agent.swingTick++;
    ...
    public void onLivingUpdate()
    {
    	...
    	//stuck in a fence/gate/pane fix, PFQueue method fix wasnt enough
    	if (!CoroUtilBlock.isAir(this.worldObj.getBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ)))) {
    		if (this.onGround) {
    			this.jump();
    		}
    	}
    ...
    @Override
	public int overrideBlockPathOffset(ICoroAI ent, Block id, int meta, int x, int y, int z) {
    	ICoroAI entAI = (ICoroAI)ent;
    	
		if (id == ZCBlocks.barricadePlaceable) {
			...
			return -2;
		} else if (id instanceof BlockBarricade) {
			if (entAI instanceof BaseEntAI_Ally) {
				return 0;
			} else {
				return 1;
			}
		} else if (id instanceof BlockSlab) {
			return 0;
		} else if (id instanceof BlockBarrier) {
			return -2;
		} else if (id instanceof BlockWallPlaceable) {
			return -2;
		} else if (id instanceof BlockTower) {
			return -2;
		}
		return -66;
    }
