...
public class Level {
	...
	public void readNBT(String filename) {
		...
		if (buildData == null) {
			if (zcGame.gameData == null) {
				...
				buildData = new Build(zcGame.gameData.getInteger("map_coord_minX"), ZCGame.ZCWorldHeight/*zcGame.gameData.getInteger("map_coord_minY")*/, zcGame.gameData.getInteger("map_coord_minZ"), filename);
			}
		} else {
		...
		if (nbttagcompound != null) {
			...
			//Camera points
			NBTTagList tag = nbttagcompound.getTagList("camPoints", 10);
			if (tag != null) {
				for (int var21 = 0; var21 < tag.tagCount(); ++var21)
	            {
	                NBTTagCompound var20 = (NBTTagCompound)tag.getCompoundTagAt(var21);
	...
	public void writeNBT() {
		...
		buildData.levelData.setTag("mapConfig", LevelConfig.saveNBT());
	...
	public List<EntityPlayer> getLobbyPlayers() {
		...
		for(int i = 0; i < players.size(); i++) {
			...
			playersInGame_Names.add(CoroUtilEntity.getName(player));
