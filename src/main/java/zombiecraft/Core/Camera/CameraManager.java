...
public class CameraManager {
	...
	public void gameTick() {
		...
		if (mc.theWorld != null/* && mc.theWorld.provider.dimensionId == ZCGame.ZCDimensionID*/) {
			if (mc.currentScreen == null && Keyboard.isKeyDown(ZCKeybindHandler.cameraKey.getKeyCode())) {
				if (!isPressed) {
					if (camState == EnumCameraState.OFF) {
						freeCam();
				...
				isPressed = true;
			} else if (mc.currentScreen == null && camState == EnumCameraState.FOLLOW && Keyboard.isKeyDown(ZCKeybindHandler.chargeKey.getKeyCode())) {
				if (!isPressed) {
					spectateNext();
				}
				isPressed = true;		
		...
		try {
			...
			if (mc.theWorld.playerEntities.size() > 1) {
				...
				for (int i = spectateTargetIndex + 1; i != spectateTargetIndex; i++) {
					...
					if (!CoroUtilEntity.getName(((EntityPlayer)spectateTarget)).equals(CoroUtilEntity.getName(entP)) && !entP.isDead && entP.deathTime == 0 && entP.getHealth() > 1) {
						System.out.println("Spectating: " + CoroUtilEntity.getName(entP));
						spectate(entP);
						spectateTargetIndex = i;
						return;
					}
