...
public abstract class ZCGame {
	...
	public void resetPlayer(EntityPlayer player, boolean teleport, boolean keepPoints) {
		...
		int curPoints = (Integer)getData(CoroUtilEntity.getName(player), DataTypes.zcPoints);
		...
		if (!player.capabilities.isCreativeMode) {
			...
			entFields.put(CoroUtilEntity.getName(player), new DataLatcher());
	...
	public void refillAmmo(EntityPlayer player) {
		ZCGame.instance().check(CoroUtilEntity.getName(player));
	...
	public void playerKillEvent(EntityLivingBase player, Entity killed) {
		...
		if (username.contains("fakePlayer")) {
			...
			if (itemStack != null && itemStack.getItem() instanceof ItemGun) {
				...
				ZCUtil.setAmmoData(CoroUtilEntity.getName(player), ammoID, giveAmount + ZCUtil.getAmmoData(CoroUtilEntity.getName(player), ammoID));
			}
	...
	public void handleBarricadeProximity(EntityPlayer player) {
    	...
    	for (int x = ((int)(player.posX-1.0))-range; x <= ((int)(player.posX+0.0))+range; x++) {
    		for (int z = ((int)(player.posZ-1.0))-range; z <= ((int)(player.posZ+0.0))+range; z++) {
    			...
    			Block id = player.worldObj.getBlock(x,adjY,z);
	...
	public void check(EntityPlayer me) {
		check(CoroUtilEntity.getName(me));
	}
	...
	public void setData(EntityPlayer ent, DataTypes dtEnum, Object obj) {
		...
		((DataLatcher)entFields.get(CoroUtilEntity.getName(ent))).values.put(dtEnum, obj);
	}
	...
	public void givePlayerItem(EntityPlayer player, Item item, int slot, boolean giveGun) {
		...
		if (item instanceof ItemBlock && ((ItemBlock)item).field_150939_a == ZCBlocks.betty) {
			count = 6;
		}
	...
	public void triggerBarricadeRepair(EntityPlayer player) {
		...
		if (((BlockBarricade)ZCBlocks.barricadeS5).isFixableBarricade(player.worldObj,x,y,z)) {
			...
			if (((BlockBarricade)id).tryRepairDoor(player.worldObj,x,y,z,player)) {
	...
	public void triggerMysteryBox(EntityPlayer player) {
		...
		TileEntity tEnt = player.worldObj.getTileEntity(x, y, z);
    ...
    public void writeGameNBT() {
    	...
    	try {
		    ...
		    while (it.hasNext()) {
		        ...
		        playerData.setTag(pairs.getKey().toString(), playerNBT);
		    }
    		gameData.setTag("playerData", playerData);
		...
		try {
			...
			if (!this.serverMode && !this.isRemote()) {
				...
				while (it.hasNext()) {
					...
					NBTTagCompound var16 = playerData.getCompoundTag(name);
	...
	public void setSpawnerWatch(EntityPlayer player, int xCoord, int yCoord, int zCoord, int x, int y, int z) {
		if (isMaster()) {
			if (isOp(player)) {
				TileEntity tileent =  player.worldObj.getTileEntity(xCoord, yCoord, zCoord);
