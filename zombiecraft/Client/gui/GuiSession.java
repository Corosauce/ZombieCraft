...
import zombiecraft.Client.gui.scrollable.GuiSlotImpl;
import zombiecraft.Client.gui.scrollable.IScrollingElement;
import zombiecraft.Client.gui.scrollable.IScrollingGUI;
...
public class GuiSession extends GuiContainer implements IScrollingGUI {
	...
	protected void drawGuiContainerForegroundLayer(int param1, int param2) {
	    ...
	    if (gameState != 0) {
	    	...
	    	strWidth = fontRenderer.getStringWidth(strTitle);
		    fontRenderer.drawString("\u00A7" + "r" + strTitle, x1 + xSize/2 - strWidth/2, y1 + 6 + (yTextSpacing * 0), 0xFFFF00);
		    strWidth = fontRenderer.getStringWidth(strTitleName);
	    ...
	    drawRect(x1-padding, y1-padding, x2+padding, y2+padding, -6250336);
	    drawRect(x1, y1, x2, y2, -16777216);
	    strWidth = fontRenderer.getStringWidth(strState);
	    fontRenderer.drawString("\u00A7" + "4" + strState, x1 + 6/*xSize/2 - strWidth/2*/, y1 + 6, 0xAAAAAA);
	    fontRenderer.drawString("\u00A7" + '4' + "Map: " + "\u00A7" + 'r' + zcGame.mapMan.curLevel, x1 + 6, y1 + 6 + (yTextSpacing * 1), 0xAAAAAA);
	    
	    String waveString = "\u00A7" + '4' + "Wave: " + "\u00A7" + '7' + zcGame.wMan.wave_Stage + "\u00A7" + "4" + " | Mobs: " + "\u00A7" + '7' + zcGame.wMan.wave_Invaders_Count;
    	strWidth = fontRenderer.getStringWidth(waveString);
    	if (gameState == 2) {
    ...
    public void initGui()
    {
        ...
        PacketHelper.sendClientPacket(ZCPacketHandler.getSessionPacket(data));
    }
	...
	public void updateScreen() {
		...
		if (username.equals(ZCGame.nbtInfoClientSession.getString("lobbyLeader"))) {
			...
			guibuttonSetMap.drawButton = false;
			guibuttonGenerate.drawButton = false;
		}
        GuiButton guibuttonTeleport = (GuiButton)this.buttonList.get(ZCPacketHandler.CMD_TELEPORT);
		if (mc.thePlayer.dimension == ZCGame.ZCDimensionID) {
			guibuttonTeleport.drawButton = true;
		} else {
			guibuttonTeleport.drawButton = false;
		}
